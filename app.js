...
function displayedCardsInFolder(folderId){const own=cardsInFolder(cards,folderId);return folderId===extraFolder(folders)?.id?[...own,...extraCards(cards,folderId)]:own;}
function render(){
...
form.onsubmit=async e=>{
  ...
  const data=new FormData(form),quantity=cardQuantity(data.get('quantity')),folderId=data.get('folder')||null,folder=folders.find(item=>item.id===folderId),title=posterTitle(data.get('title'),folder),year=data.get('year').trim(),links=urls(),files=[...gallery.files];
