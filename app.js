...
folderForm.onsubmit=async e=>{
  ...
  $('#folderError').textContent='Creating…';const folder={id:crypto.randomUUID(),user_id:session.user.id,name,release_year:year?Number(year):null,auto_name:$('#autoNameFolder').checked,sort_order:folders.length};const {error}=await supabase.from('folders').insert(folder);
  if(error){$('#folderError').textContent=error.message;return;}folders.push({id:folder.id,name,year,autoName:folder.auto_name,sort_order:folder.sort_order});folderDialog.close();render();
};

form.onsubmit=async e=>{
  e.preventDefault();if(!session)return openAuth();
  const data=new FormData(form),folderId=data.get('folder')||null,folder=folders.find(item=>item.id===folderId),title=posterTitle(data.get('title'),folder),year=data.get('year').trim(),links=urls(),files=[...gallery.files];
